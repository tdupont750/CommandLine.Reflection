...
readonly CLI_APP="$CLI_DIR/apps"
readonly CLI_LSH="$CLI_DIR/lshash.dll"
readonly CLI_ALI=~/.bashrc
readonly CLI_PROJ_NAM="$(basename $0 .sh)"
readonly CLI_PROJ_DIR="$CLI_APP/$CLI_PROJ_NAM"
...
then
	...
	if [[ $(cat "${CLI_ALI}" | grep "alias $CLI_PROJ_NAM=" | wc -l) = "1" ]]
	then
	...
	# write alias
	echo '' >> "${CLI_ALI}"
	echo "alias $CLI_PROJ_NAM='./$CLI_PROJ_NAM.sh \$@'" >> "${CLI_ALI}"
...
# check if print log flag is set
if [[ $1 == "--print-log" ]]
then
